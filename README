...
This repository contains the source for the open book used in teaching applied statistics for health professionals at the School of Health Professions of the Bern University of Applied Sciences. To read the book visit [https://bern-movement-lab.github.io/statbook/](https://bern-movement-lab.github.io/statbook/).
...
``` shell
git clone git@github.com:bern-movement-lab/statbook.git
```

Make changes as desired and render the book locally:

``` shell
quarto render
```

The book will be rendered and the results from computations are stored in the `_freeze` directory, which must be committed to the repository. The book is published via the [GitHub Pages](https://docs.github.com/en/pages) website hosting service from GitHub and with the help of [Quarto GitHub Pages publishing](https://quarto.org/docs/publishing/github-pages.html). Quarto offers three ways of how to publish Quarto websites and documents to GitHub Pages. This project uses a GitHub Action to automatically render the files and publish the resulting content whenever changes in the source code are pushed to the repository. However, local rendering as previously described is necessary for the executable code in the Quarto documents, since this is not implemented to be overtaken as part of the GitHub Action.
